...
//import dbFactory from 'firebase';
...
export const getTeacherNotificationFromDB = (dispatch, uid) => {
    ...
    const notifications = [];
    const db = dbFactory.create('firebase');
    db.firestore().collection('notifications').where("tId", "==", uid).where("deleted", "==", false).get()
        .then((querySnapshot) => {
...
export const getNotificationFromDB = (dispatch, uid) => {
    ...
    const notifications = [];
    const db = dbFactory.create('firebase');
    db.firestore().collection('notifications').where("nId", "==", uid).where("deleted", "==", false).get()
        .then((querySnapshot) => {
            querySnapshot.docs.forEach(doc => {
                notifications.push(doc.data())
            });
...
export const getTeacherFromStudentIdFromDB = (dispatch, uid) => {
    ...
    const notifications = [];
    const db = dbFactory.create('firebase');
    db.firestore().collection('notifications').where("sId", "==", uid).where("deleted", "==", false).get()
        .then((querySnapshot) => {
            querySnapshot.docs.forEach(doc => {
                notifications.push(doc.data())
            });
...
export const saveChatNotificationDetails = chatNotificationDetails => {
    ...
    const db = dbFactory.create('firebase');
    db.firestore().collection('notifications').doc(chatNotificationDetails.nId).set(chatNotificationDetails).then(() => {
...
export const udpateChatNotificationDetails = chatNotificationDetails => {
    ...
    const db = dbFactory.create('firebase');
    db.firestore().collection('notifications').doc(chatNotificationDetails.nId).update(chatNotificationDetails).then(() => {
